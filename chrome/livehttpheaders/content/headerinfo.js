...
if (!livehttpheaders.info) livehttpheaders.info = {};
...
livehttpheaders.info.saveAs = function (data, title) {
    ...
    try {
        var picker =
            Components.classes[PICKER_CTRID].createInstance(nsIFilePicker);
        ...
        if (rv != Components.interfaces.nsIFilePicker.returnCancel) {
            var os =
                Components.classes[FILEOUT_CTRID].createInstance(
                    nsIFileOutputStream
                );
