...
livehttpheaders.panel.SIDEBAR_CID = Components.ID(
...
livehttpheaders.panel.DIR_SERV_CONTRACTID =
    "@mozilla.org/file/directory_service;1";
livehttpheaders.panel.STD_URL_CONTRACTID =
    "@mozilla.org/network/standard-url;1";
livehttpheaders.panel.NETSEARCH_CONTRACTID =
...
livehttpheaders.panel.mySidebar = function () {
    ...
    const PANELS_RDF_FILE =
        "UPnls"; /* directory services property to find panels.rdf */
...
livehttpheaders.panel.mySidebar.prototype.nc =
    "http://home.netscape.com/NC-rdf#";

/* decorate prototype to provide ``class'' methods and property accessors */
livehttpheaders.panel.mySidebar.prototype.addPanel = function (
    winurl,
    winname,
    msgadded,
    msgpresent
    ...
    var panel_list = this.datasource.GetTarget(
        this.rdf.GetResource(this.resource),
        this.rdf.GetResource(
            livehttpheaders.panel.mySidebar.prototype.nc + "panel-list"
        ),
        true
    );
    panel_list.QueryInterface(Components.interfaces.nsIRDFResource);

    var container = Components.classes[
        livehttpheaders.panel.CONTAINER_CONTRACTID
    ].createInstance(Components.interfaces.nsIRDFContainer);
    container.Init(this.datasource, panel_list);

    /* Create a resource for the new panel and add it to the list */
    var panel_resource = this.rdf.GetResource(
        "urn:sidebar:3rdparty-panel:" + aContentURL
    );
...
function getSidebarDatasourceURI(panels_file_id) {
    try {
        ...
        if (directory_service) {
            directory_service = directory_service.QueryInterface(
        ...
        var sidebar_file = directory_service.get(
            panels_file_id,
            Components.interfaces.nsIFile
        );

        var io_service = Components.classes[
            "@mozilla.org/network/io-service;1"
        ].getService(Components.interfaces.nsIIOService);
        var url = io_service
            .newFileURI(sidebar_file)
